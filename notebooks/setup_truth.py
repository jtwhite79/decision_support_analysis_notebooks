...
def run_draws_and_pick_truth(run=True,num_workers=10,t_d="template_truth",m_d="master_truth_sweep"):
...
def build_model(redis_fac=1,tr_d="temp_truth"):
    ...
    _ = flopy.modflow.ModflowDis(m_tr,nper=len(perlen),nlay=m_org.nlay,nrow=m_org.nrow,ncol=m_org.ncol,delr=m_org.dis.delr.array,
    ...
    for kper in range(1,m_tr.nper):
        ...
        kper_wel_data["flux"] *= 1.2
        wel_data[kper] = kper_wel_data
        rech[kper] = org_rch * 0.9
    ...
    for k in range(m_tr.nlay):
        np.savetxt(os.path.join(tr_d,"prsity_layer_{0}.ref".format(k+1)),np.zeros((m_tr.nrow,m_tr.ncol))+0.001,fmt="%15.6E")
...
def setup_interface_truth(b_d="temp_truth",t_d="template_truth"):
    ...
    prep_deps.prep_template(t_d=b_d)
    ...
    pst_helper = pyemu.helpers.PstFromFlopyModel(nam_file,new_model_ws=t_d,org_model_ws=b_d,
...
if __name__ == "__main__":
    #revise_base_model()
    build_model(redis_fac=1)
    setup_interface_truth()
    run_draws_and_pick_truth(run=True,num_workers=8)
